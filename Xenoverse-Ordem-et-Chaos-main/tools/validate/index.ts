...
import { getDb } from '../apps/dex/src/lib/db/connection';
 ...
 */
function validateCanonical(db: any): ValidationIssue[] {
  ...
  `).all();
    ...
    // 5. Check for WIP/dev items (those with empty names or specific flags)
    const devItems = db.prepare(`
    SELECT id, name, flags FROM items 
    WHERE name LIKE '%WIP%' OR name LIKE '%TODO%' OR name LIKE '%???%'
  `).all();

    for (const row of devItems) {
        issues.push({
            level: 'ERROR',
            layer: 'canonical',
            entity: `items:${row.id}`,
            field: 'name',
            message: `Possible dev/placeholder item: ${row.name}`,
            details: row
        });
    }
 ...
 */
function validateDerived(db: any): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
...
export function runValidation() {
    ...
    const outDir = join(process.cwd(), 'out', 'builds', 'latest');
    ...
    // Write JSON report
    writeFileSync(
        join(outDir, 'validation_report.json'),
        JSON.stringify(report, null, 2)
    );

    // Write Markdown report
    const md = generateMarkdownReport(report);
    writeFileSync(join(outDir, 'validation_report.md'), md);
